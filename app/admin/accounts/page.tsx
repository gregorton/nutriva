...
/*
  ...
  This is the one surface on the site that displays an email address — the reason lib/admin.ts
  keeps the gate to an allowlist and answers a stranger with a 404 rather than a 403.
*/
export default async function AdminAccountsPage({ searchParams }: PageProps<"/admin/accounts">) {
  ...
  const page = await recentAccounts(typeof before === "string" ? before : null);
  ...
  return (
    <section>
      <RankTable
        ...
        rows={page.rows.map((account) => ({
          ...
          cells: [
            ...
            [account.hasPassword ? "password" : null, ...account.providers]
              .filter(Boolean)
              .join(" + ") || "—",
      ...
      <Pager
        base="/admin/accounts"
        cursor={typeof before === "string" ? before : null}
